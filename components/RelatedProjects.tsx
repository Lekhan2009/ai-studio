...
const RelatedProjects = async ({ userId, projectId }: Props) => {
    const result = await getUserProjects(userId) as { user?: { projects?: { edges: { node: ProjectInterface }[] } } }
    ...
    return (
        <section className="flex flex-col mt-32 w-full">
            ...
            <div className="related_projects-grid">
                {filteredProjects?.slice(0, 4).map(({ node }: { node: ProjectInterface }) => (
                    <div className="flexCenter related_project-card drop-shadow-card" key={`${node?.id}`}>
                        <Link href={`/project/${node?.id}`} className="flexCenter group relative w-full h-full">
                            <Image src={node?.image} width={414} height={314} className="w-full h-full object-cover rounded-2xl" alt="project image" />

                            <div className="hidden group-hover:flex related_project-title">
                                <p className="w-full">{node?.title}</p>
                            </div>
                        </Link>
                    </div>
